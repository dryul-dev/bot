...
class PveBattle:
    ...
    async def timeout_task(self):
        try: await asyncio.sleep(300); await self.end_battle(win=False, reason="사냥 시간이 너무 오래 걸려 집중력을 잃었습니다...")
    ...
    async def monster_turn(self):
        monster = self.monster_stats; player = self.player_stats; action_roll = random.random(); log_message = ""; initial_defense = player.get('pve_defense', 0)
        if action_roll < 0.6:
            damage = max(1, monster['ap'] + random.randint(-monster['level'], monster['level'])); final_damage = max(0, damage - initial_defense); player['pve_defense'] = max(0, initial_defense - damage); player['current_hp'] = max(0, player['current_hp'] - final_damage); log_message = f"👹 **{monster['name']}**의 공격! **{player['name']}**에게 **{final_damage}**의 피해!"
        elif action_roll < 0.9: defense_gain = round(monster['hp'] * 0.2); monster['defense'] += defense_gain; log_message = f"🛡️ **{monster['name']}**이(가) 방어 태세를 갖춥니다! (방어도 +{defense_gain})"
        else:
            damage = max(1, monster['ap'] + random.randint(-monster['level'], monster['level'])) * 2; final_damage = max(0, damage - initial_defense); player['pve_defense'] = max(0, initial_defense - damage); player['current_hp'] = max(0, player['current_hp'] - final_damage); log_message = f"💥 **{monster['name']}**의 강한 공격! **{player['name']}**에게 **{final_damage}**의 치명적인 피해!"
        if initial_defense > 0 and '피해' in log_message: log_message += f" (방어도 {initial_defense} → {player['pve_defense']})"
        if player['current_hp'] <= 0:
            await self.channel.send(embed=discord.Embed(description=log_message, color=0xDC143C)); await asyncio.sleep(1); await self.end_battle(win=False, reason=f"{monster['name']}의 공격에 쓰러졌습니다..."); return
        if player.get('special_cooldown', 0) > 0: player['special_cooldown'] -= 1
        self.current_turn = "player"; embed = discord.Embed(title="몬스터의 턴 결과", description=log_message, color=player['color']); embed.add_field(name=f"{player['name']}", value=f"HP: {player['current_hp']}/{player['hp']}", inline=True); embed.add_field(name=f"{monster['name']}", value=f"HP: {monster['current_hp']}/{monster['hp']}", inline=True); embed.set_footer(text="▶️ 당신의 턴입니다."); await self.channel.send(embed=embed); await self.start_turn_timer()
